...
public class FourthSkill extends CollectSkill {
    ...
    public List<String> getInfo(int level) {
        ...
        if (level == 1) {
            lore.add(" §8▪ §7你有§a" + StringUtils.percent(this.getAttribute(level)) + "§7的几率在箱子里找到生命恢复III(4秒)药水");
            return lore;
        } else {
            lore.add(" §8▪ §7你有§8" + StringUtils.percent(this.getAttribute(level - 1)) + " ➜ §a" + StringUtils.percent(this.getAttribute(level)) + "§7的几率在箱子里找到生命恢复III(4秒)药水");
            return lore;
        }
    }
